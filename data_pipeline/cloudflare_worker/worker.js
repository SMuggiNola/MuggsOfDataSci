  ...
  async fetch(request, env) {
    ...
    if (url.pathname.endsWith("/save") && request.method === "POST") {
      try {
        ...
        await env.MUGGS_DATA.put(key, JSON.stringify(data));
        ...
        return new Response(`Error saving: ${err}`, { status: 500 });
      }
    }
    ...
    if (url.pathname.endsWith("/load") && request.method === "GET") {
      ...
      const stored = await env.MUGGS_DATA.get(key);
    ...
    // ====== 3️⃣ DEFAULT ROUTE ======
    return new Response("Muggs Data Worker active. Use /save or /load.", {
      status: 200
    });
  }
};
